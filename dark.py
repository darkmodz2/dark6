...
def start(message):
    ...
    markup.row('🚀 Start Attack', '✅ My Account')
...
def my_account(message):
    ...
    if is_user_authorized(user_id):
        bot.send_message(message.chat.id, "*Access: Active ✅*", parse_mode='Markdown')
    else:
        bot.send_message(message.chat.id, "*Access: Inactive ❌*\n\n*Buy Key to activate.*\n\n"
                                          "*KEY PRICE LIST* 🔐\n"
                                          "⏳ 1 HOUR = ₹10\n"
                                          "⏳ 1 DAY = ₹99\n"
                                          "⏳ 7 DAYS = ₹399\n\n"
                                          "*Contact Owner 👉 @Darknetdon1*", parse_mode='Markdown')
...
def handle_buy_key(message):
    bot.send_message(message.chat.id,
                     ...
                     "⏳ 1 DAY = ₹99\n"
                     "⏳ 7 DAYS = ₹399\n\n"
                     "*Contact Owner 👉 @Darknetdon1 to buy key.*", parse_mode='Markdown')
